$(function(){
    let usersCheck = get_data_localStorage("users");
    if (usersCheck.length == 0) {
      ...
      [
       {
         ...
         "age":"24",
           "date":"2022-01-14",
         "email":"Mahmoud.M.Mansourr@gmail.com",
         "address":"Kafr Elsheikh",
         "role":"admin",
         "status":true,
       },
       {
     ...
     set_data_localStorage("users",users);
    }
...
function SendLogin(employee) {
    if (employee) {
        Email.send({
            SecureToken : "bad0a7fc-2d52-4a11-be2a-15fce1c3daeb",
            To : employee.email,
            From : "iti_js_project@outlook.com",
...
function leaveAll() {
        ...
        let attendance = get_data_localStorage("attendance");
        let dateNow = new Date();
        ...
        set_data_localStorage("attendance",attendance);
}
...
function set_data_localStorage(key,value) {
...
function addUser(newUser) {
  let users = get_data_localStorage("users");
  users.push(newUser);
  set_data_localStorage("users",users);
}
...
function userCheck(empUsername , empPassword){
    if (user_exists(empUsername)) {
        let emp = getUserData(empUsername);
        if (empPassword == emp["password"]){
            if (emp["status"] == true){
                set_data_localStorage("session" , emp);
...
function getEmployeeAttendance(username) {
    ...
    let dateNow = new Date();
    let attendanceData = get_data_localStorage("attendance");
...
function addArrivalAttendance(username) {
    ...
    set_data_localStorage("attendance",attendanceData);
